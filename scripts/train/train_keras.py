...
def create_model(model_name,conv_n,conv_w,dr,reg):
    if model_name == 'logit':
        model = Sequential()
        model.add(Reshape((160,),input_shape=(40,4)))
        model.add(Dropout(dr))
        model.add(Dense(1, activation='sigmoid', kernel_regularizer=l1_l2(l1=reg, l2=reg)))
...
def train_model(model, train_seq, val_seq, lr, epochs):
    ...
    return model.fit_generator(train_seq,epochs=epochs,shuffle=True,validation_data=val_seq,callbacks=[EarlyStopping()])

def evaluate_model(model, val_seq, outfile):
    """Calculate accuracy"""
    val_acc = model.evaluate_generator(val_seq)
    print(val_acc)
    return val_acc

def predict_model(model, test_seq, outfile):
    prediction = model.predict_generator(test_seq)
    np.save(outfile + '_predictions',prediction)    
...
def main():
    ...
    parser.add_argument('--reg',type=float,action='store',default=0.01,help='Regularisation')
    ...
    if args.test:
        model = create_model(args.model_type,args.conv_n,args.conv_w,args.dr,args.reg)
        test_seq = HDF5Sequence(x_test,y_test,128)
        predict_model(model,test_seq,args.outfile)
        np.save(args.outfile + '_test',y_test[:])
        
    elif args.crossvalid:
