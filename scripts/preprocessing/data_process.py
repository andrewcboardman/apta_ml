...
def merge_fasta(infile1, infile2, outfile):
    os.system('cat {} {} | perl /storage/code/seq-shuf.pl > {}'.format(infile1,infile2,outfile))
...
def main():
    ...
    label_fasta(args.infile1, args.infile1 + '_lbl',1)

    print('Labelling control sequences...')

    label_fasta(args.infile2, args.infile2 + '_lbl',0)

    print('Merging binding and control sequences...')

    merge_fasta(args.infile1 + '_lbl',args.infile2 + '_lbl',args.infile1 + '_merge')

    print('Writing encoded sequences to hdf5 format...')

    to_hdf5(args.infile1 + '_merge',args.outfile)
