...
model = load_model('models/model_1/model_1')
...
split = np.genfromtxt('models/model_1/model_1_split.txt')
...
np.savetxt('models/model_1/model_1_test_pred.txt',np.stack((y_test,y_pred)).T)
...
plt.xlabel('FPR')
plt.ylabel('TPR')
plt.title(f'ROC for classification using CNN (model 1)\n AUC = {auc}')
plt.savefig('test_roc_model_1')
